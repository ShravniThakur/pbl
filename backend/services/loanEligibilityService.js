...
const generateOffer = (requestedLoanAmount, loanType, eligibilityScore, riskScore) => {
    ...
    return { maxApprovedLoanAmount: approvedAmount, maxApprovedTenureMonths: approvedTenure, maxApprovedInterestRatePercent: approvedRate, emi };
};
...
const runRuleEngine = (profile, requestedLoanAmount, loanType, loanDetails) => {
    ...
    if (loanType !== "Education Loan" && income > 0) {
        const estimatedNewEMI = calculateEMI(requestedLoanAmount, LOAN_TYPE_CONFIG[loanType].baseRate, LOAN_TYPE_CONFIG[loanType].maxTenureMonths);
...
const createLoanEligibilityCheckService = async (userID, data) => {
    ...
    const defaultTenure = { "Personal Loan": 60, "Education Loan": 120, "Home Loan": 240, "Vehicle Loan": 84, "Business Loan": 60 };
    const tenureMonths = defaultTenure[loanType];
    const hasCoApplicant = !!loanDetails.coApplicant;

    const { eligibilityScore, riskScore, mlPrediction, mlAvailable } = await computeMLScores(profile, requestedLoanAmount, tenureMonths, hasCoApplicant);
    ...
    if (mlAvailable) {
        try {
            ...
            const { data: shapData } = await axios.post((process.env.SHAP_SERVICE_URL || 'http://localhost:8001') + '/explain', payload, { timeout: 15000 });
    ...
    let results = { eligible, rejectionReasons, eligibilityScore: eligible ? eligibilityScore : null, riskScore: eligible ? riskScore : null, riskCategory: null, maxApprovedLoanAmount: null, maxApprovedTenureMonths: null, maxApprovedInterestRatePercent: null, emi: null };

    if (eligible) {
        const riskCategory = riskScore <= 20 ? "Very Low" : riskScore <= 40 ? "Low" : riskScore <= 60 ? "Medium" : riskScore <= 80 ? "High" : "Very High";
